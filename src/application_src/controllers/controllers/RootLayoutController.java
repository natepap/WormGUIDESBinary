...
public class RootLayoutController extends BorderPane implements Initializable {
    ...
    @FXML
    public void viewBinTreeAction(){
        viewTree(true);
    }

    @FXML
    public void viewTree(boolean isBin) {
        if (lineageData.isSulstonMode()) {
            if (sulstonTreeStage == null) {
                ...
                final SulstonTreePane treePane = new SulstonTreePane(
                        ...
                        annotationManager,
                        binLineageMap);
                treePane.setIsBin(isBin);
                sulstonTreeStage.setScene(new Scene(treePane));
                if(isBin){
                    sulstonTreeStage.setTitle("Binary Lineage Tree");
                }else{
                    sulstonTreeStage.setTitle("Lineage Tree");
                }
                ...
                if((sulstonTreeStage.getTitle() == "Binary Lineage Tree" && !isBin) ||
                        sulstonTreeStage.getTitle() == "Lineage Tree" && isBin) {
                    sulstonTreeStage.close();
                    sulstonTreeStage = null;
                    viewTree(isBin);
                }
                sulstonTreeStage.show();
            ...
            if (functionalName != null) {
                if(hasBinName){
                    displayedName.setText("Active Cell: " + name + " (" + functionalName + ")  " +
                            binLineageMap.get(name.toLowerCase()).getValue());
                }else{
                    displayedName.setText("Active Cell: " + name + " (" + functionalName + ")");
                }
    ...
    private void initLineageTree(final List<String> allCellNames) {
        ...
        binLineageMap = lineageTree.getBinMap();
    }
