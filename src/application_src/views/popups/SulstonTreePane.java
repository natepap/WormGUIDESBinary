...
public class SulstonTreePane extends ScrollPane {
    ...
    public SulstonTreePane(
            ...
            final Map<String, TreeItem<String>> binLineageMap) {
    ...
    private int recursiveDraw(
        ...
        if (cell.isLeaf() || done) {
            ...
            final Text cellnametext;

            // use binary names on tree if isBin
            if(isBin) {
                 cellnametext = new Text(x - offsetx, yStartUse + length + offsety,
                        binLineageMap.get(cellName.toLowerCase()).getValue());
            }else{
                cellnametext = new Text(x - offsetx, yStartUse + length + offsety, cellNameTextString);
            }
    ...
    public void setIsBin(boolean isBin){this.isBin = isBin;}
